...
def interval_between(note1, note2):
    harmonic_distance = harmonic_distance_between(note1, note2)
    interval_number = interval_number_between(note1, note2)
    is_descending_interval = harmonic_distance < 0
    if is_descending_interval:
        harmonic_distance = neg(harmonic_distance)
        interval_number = neg(interval_number)
    possible_intervals = INTERVALS[harmonic_distance % NUMBER_OF_CHROMATIC_NOTES]
    quality = search(
        f"(.{str(interval_number % NUMBER_OF_DIATONIC_NOTES)})",
        possible_intervals,
    )[0][0]
    return ("-" if is_descending_interval else "") + quality + str(interval_number)
